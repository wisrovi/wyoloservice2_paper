...
Todas las operaciones se ejecutan después del entrenamiento en los flujos objetivo candidatos antes de la aprobación de salida del modelo. Para facilitar la trazabilidad y la reproducibilidad, los siete pares de dominios reales evaluados en este trabajo están mapeados con sus estadísticas de distancia empírica en el archivo \texttt{domain\_pairs.npz} publicado: Par 0 (línea base de desplazamiento leve, $\text{FID}=18.3$), Par 1 (Sintético$\rightarrow$DíaReal, $\text{FID}=43.8$), Par 2 (DíaReal$\rightarrow$Lluvia, $\text{FID}=72.3$), Par 3 (Despejado$\leftrightarrow$Lluvioso, $\text{FID}=90.74$), Par 4 (Sintético$\rightarrow$Noche, $\text{FID}=127.6$), Par 5 (Día$\leftrightarrow$Noche, $\text{FID}=142.93$), y Par 6 (Interior$\leftrightarrow$Exterior, $\text{FID}=149.86$).
