...
\begin{enumerate}
    ...
    \item \textbf{Statistical calibration} via 1,000-iteration bootstrap, producing 95\% confidence intervals on every prediction. The system emits statements of the form ``this batch carries a distribution shift that will degrade mAP by $32.9$ pp (95\% CI: $31.4$--$34.2$ pp)'', with no labeled target data.

    \item \textbf{A conservative scene-complexity option}: augmenting the regression with an SC index (edge density, mean detections per image) raises interval coverage from 57.1\% to 100\% and trims leave-one-out MAE from 1.45 to 1.17 pp. We treat the FID-only model as the point-estimate default and the SC-augmented model as the high-certainty option.
...
\scriptsize
\vspace{4pt}
\texttt{[Target Images] $\to$ [InceptionV3 Feature Bank] $\to$ [Gaussian Fit ($\mu_T, \Sigma_T$)]} \\
\texttt{$\downarrow$} \\
\texttt{[Fr\'{e}chet Inception Distance (FID) vs Source ($\mu_S, \Sigma_S$)]} \\
\texttt{$\downarrow$} \\
\texttt{[Scene Complexity (SC) Index (Edge density $\bar{E}$ + decoupled baseline detections $\bar{O}$)]} \\
\texttt{$\downarrow$} \\
\texttt{[OLS Regression \& 1000-iteration Bootstrap CI Bounds]} \\
\texttt{$\downarrow$} \\
\texttt{[Gating Threshold check $\to$ Machine-readable JSON risk flag]}
\vspace{4pt}
\end{minipage}
}
\caption{Predictive pipeline flow diagram for downstream performance estimation.}
\label{fig:pipeline}
\end{figure}

All operations are executed post-training on the candidate target streams before model gate approval. To facilitate traceability and reproducibility, the seven real domain pairs evaluated in this work are mapped to their empirical distance statistics in the released \texttt{domain\_pairs.npz}: Pair 0 (Mild-shift baseline, $\text{FID}=18.3$), Pair 1 (Synthetic$\rightarrow$RealDay, $\text{FID}=43.8$), Pair 2 (RealDay$\rightarrow$Rain, $\text{FID}=72.3$), Pair 3 (Clear$\leftrightarrow$Rainy, $\text{FID}=90.74$), Pair 4 (Day$\rightarrow$Night, $\text{FID}=127.6$), Pair 5 (Synthetic$\rightarrow$Night, $\text{FID}=142.93$), and Pair 6 (Indoor$\leftrightarrow$Outdoor, $\text{FID}=149.86$).
