...
Enforcing the interval-based gate (lower bound $> 10$ pp) on the FID-only model flags exactly 5 pairs (RealDay$\rightarrow$Rain, Clear$\leftrightarrow$Rainy, Synthetic$\rightarrow$Night, Day$\leftrightarrow$Night, and Indoor$\leftrightarrow$Outdoor) and never flags the 2 low-risk pairs (the mild-shift baseline and Synthetic$\rightarrow$RealDay), though the actual degradation of one unflagged pair (Synthetic$\rightarrow$RealDay) is $11.03$ pp (with a conservative LOO CI lower bound of $7.52$ pp, keeping it safely below the $10$ pp risk threshold). This validates the statistical gatekeepers' conservative behavior.
