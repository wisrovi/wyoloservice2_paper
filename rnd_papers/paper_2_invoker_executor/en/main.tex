...
\begin{figure}[htbp]
\centering
% MERMAID_DIAGRAM_PLACEHOLDER: Flowchart showing the Invoker-Executor isolation. The Invoker (Celery) receives task -> Spawns Ephemeral Docker (Executor) with hard limits -> Executor runs PyTorch/YOLO. If OOM occurs, the Container Dies, but the Invoker survives and reports the crash. The subagent MUST design this diagram using subgraphs to ensure it is visually balanced and square-like, fitting perfectly within a SINGLE column.
\end{figure}
