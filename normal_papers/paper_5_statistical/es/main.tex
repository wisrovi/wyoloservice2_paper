...
Todos los experimentos se realizaron sobre el conjunto de datos COCO128. Adoptamos una partición de datos 80/20 para asegurar que la estimación de la varianza se calcule sobre un conjunto de prueba estrictamente no visto ($N=128$ imágenes de validación) utilizando un tamaño de batch de 16 y una resolución de entrada (\texttt{imgsz}) de 640. Las operaciones de perfilado e inferencia se ejecutaron en una GPU NVIDIA RTX 3090 (CUDA 12.1). Notamos explícitamente que, si bien el bootstrap sobre un único conjunto de prueba captura la varianza de muestreo del dataset, no captura completamente la varianza de re-entrenamiento \cite{bosma2024statistical}. El pipeline de evaluación se automatizó utilizando \texttt{benchmark\_statistical.py}.

\section{Resultados Experimentales}

\subsection{Significancia Estadística de las Ganancias de mAP}
Evaluamos tres variantes de YOLO (YOLO-n, YOLO-s, YOLO-m) contra un YOLO-baseline estándar. Como se muestra en la Tabla \ref{tab:bootstrap}, adoptamos el valor $p$ de permutación ($p < 0.05$) como el criterio de decisión primario, mientras que la superposición de Intervalos de Confianza sirve como una métrica descriptiva secundaria. YOLO-n logró una estimación de punto más alta (0.8311 vs 0.8224) y demostró una mejora estadísticamente significativa ($p = 0.0152$). Sin embargo, su IC del 95\% [0.8245, 0.8380] se superpuso ligeramente con el límite superior de la línea base, resaltando que la superposición del IC por sí sola es un proxy estrictamente conservador en comparación con la prueba de permutación emparejada formal. YOLO-m demostró una mejora inequívoca ($p < 0.0001$), justificando definitivamente su despliegue a pesar de los mayores costos computacionales.
...
Para validar el mecanismo Bootstrap, realizamos un estudio de ablación mediante 500 ensayos simulados de despliegue A/B, en los cuales la línea base y el modelo candidato compartían distribuciones poblacionales idénticas. Confiar únicamente en estimaciones de punto resultó en una tasa del 49.5\% de despliegue de modelos que en realidad no ofrecían ninguna mejora real (esencialmente un lanzamiento de moneda). La implementación de la puerta de IC del 95\% ($p < 0.05$) redujo esta tasa de despliegues falsos positivos a exactamente el 2.0\%, limitando de forma segura el riesgo de despliegue por debajo de la tasa teórica nominal de error de Tipo I ($\alpha = 0.05$). Esta calibración conservadora se alinea con los hallazgos de Bosma et al. \cite{bosma2024statistical}.
