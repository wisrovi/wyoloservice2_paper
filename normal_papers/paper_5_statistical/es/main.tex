...
Evaluamos tres variantes de YOLO (YOLO-n, YOLO-s, YOLO-m) contra un YOLO-baseline estándar \cite{redmon2016you}. Como se muestra en la Tabla \ref{tab:bootstrap}, adoptamos el valor $p$ de permutación ($p < 0.05$) como el criterio de decisión primario, apoyándonos en los principios discutidos por Salzberg \cite{salzberg1997comparing}. YOLO-n logró una estimación de punto más alta (0.6138 vs 0.6070) pero falló en demostrar una mejora estadísticamente significativa ($p = 0.5161$), haciendo que su despliegue sea injustificable. YOLO-m demostró una mejora inequívoca ($p < 0.0001$), justificando definitivamente su despliegue a pesar de los mayores costos computacionales.
...
El \texttt{OutlierFailureAnalyzer} analizó el conjunto de validación y aisló errores sistemáticos. La Tabla \ref{tab:failure} detalla las causas principales de cada modo de fallo. El mayor volumen de errores provino de los Falsos Positivos (10 instancias).

\begin{table}[htbp]
\caption{Taxonomía de Modos de Fallo (COCO128)}
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
\textbf{Modo de Fallo} & \textbf{Conteo} & \textbf{Descripción} \\
\hline
Falsos Positivos & 10 & Clutter de fondo \\
Detecciones Perdidas & 0 & Oclusión pesada \\
Regresión de Cajas & 0 & Aspect ratios extremos \\
Confusión de Clase & 0 & Similitud visual \\
\hline
\end{tabular}
\label{tab:failure}
\end{center}
...
Para validar el mecanismo Bootstrap, realizamos un estudio de ablación mediante 500 ensayos simulados de despliegue A/B con múltiples semillas, en los cuales la línea base y el modelo candidato compartían distribuciones poblacionales idénticas. Confiar únicamente en estimaciones de punto resultó en una tasa del 50.6\% de falsos positivos. La implementación de la puerta de IC del 95\% ($p < 0.05$) redujo esta tasa de despliegues falsos positivos al 4.2\%, limitando el riesgo de despliegue por debajo de la tasa de error Tipo I nominal ($\alpha = 0.05$). Esta calibración conservadora valida empíricamente los hallazgos de Bosma et al. \cite{bosma2024statistical}.
