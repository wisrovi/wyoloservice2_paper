...
def tex_to_md(filepath):
    ...
    # Convert figures to markdown
    content = re.sub(r"\\begin\{figure\}.*?\\includegraphics(?:\[.*?\])?\{(.*?)\}.*?\\caption\{(.*?)\}.*?\\end\{figure\}", r"![\2](\1)\n", content, flags=re.DOTALL)
    content = re.sub(r"\\begin\{figure\}.*?\\end\{figure\}", "", content, flags=re.DOTALL)
    
    # Tables are too complex for simple regex, leave them or convert loosely
    # For now, let's keep the content inside the tabular if possible
    # Just removing the begin/end and keeping the text
    content = re.sub(r"\\begin\{table\}.*?\\begin\{tabular\}\{.*?\}(.*?)\\end\{tabular\}.*?\\end\{table\}", r"\n\1\n", content, flags=re.DOTALL)
    ...
    content = re.sub(r"\\IEEEoverridecommandlockouts", "", content)
    content = re.sub(r"\n{3,}", "\n\n", content)
