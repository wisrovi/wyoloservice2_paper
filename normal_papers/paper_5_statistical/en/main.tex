...
To validate the Bootstrap mechanism, we conducted an ablation study via 500 simulated A/B deployment trials across multiple seeds, where the baseline and candidate model shared identical population distributions. Relying solely on point estimates resulted in a 49.5\% false positive rate (essentially a coin flip). Implementing the 95\% CI gating ($p < 0.05$) reduced this measured false-positive deployment rate to 2.2\%, securely bounding the risk below the nominal Type I error rate ($\alpha = 0.05$). This conservative calibration empirically confirms findings by Bosma et al. \cite{bosma2024statistical}.
