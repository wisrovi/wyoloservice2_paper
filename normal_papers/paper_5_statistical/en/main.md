...
### Ablation Study
To validate the Bootstrap mechanism, we conducted an ablation study via 500 simulated A/B deployment trials where the baseline and candidate model shared identical population distributions. Relying solely on point estimates resulted in a 49.5\% rate of deploying models that actually offered no real improvement (essentially a coin flip). Implementing the 95\% CI gating ($p < 0.05$) reduced this measured false-positive deployment rate to exactly 2.0\%, safely bounding the deployment risk below the nominal theoretical Type I error rate ($\alpha = 0.05$). This conservative calibration aligns with findings by Bosma et al. .
