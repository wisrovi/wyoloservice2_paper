...
Métodos como Grad-CAM \cite{selvaraju2017grad} y Grad-CAM++ \cite{chattopadhay2018grad} ofrecen explicaciones visuales, mientras que RISE \cite{petsiuk2018rise} introduce métricas basadas en perturbaciones. Extendemos esto aplicando métricas rigurosas y reducción de dimensionalidad con t-SNE \cite{vandermaaten2008tsne}.

\section{Metodología}
Nuestro pipeline comprende cuatro componentes centrales:
1. \textbf{ImageECamYOLO}: Genera mapas Eigen-CAM y Grad-CAM++.
2. \textbf{QuantitativeXAIValidator}: Calcula AUC de Eliminación e Inserción.
3. \textbf{FeatureRepresentationAnalyzer}: Extrae embeddings y los mapea con t-SNE.
4. \textbf{LlmAnalyzer}: Usa OpenCode LLM para generar explicaciones narrativas.
...
Los scripts y los resultados empíricos estrictamente ejecutados en CSV (p. ej., \texttt{results\_xai\_deletion.csv}, \texttt{results\_xai\_insertion.csv}, \texttt{results\_tsne\_clusters.csv}) están publicados en la carpeta \texttt{evidencias/} de este paper. Este ecosistema opera bajo Licencia Dual (PolyForm Noncommercial / AGPLv3, compatible con las normativas de publicación de IEEE). Código en \url{https://github.com/wisrovi/}. Los comandos de reproducción son: \texttt{python benchmark\_xai\_fidelity.py} para la generación de métricas. Las versiones en español e inglés de este manuscrito se mantienen sincronizadas a través del script \texttt{fix.py}, garantizando una única fuente de verdad.
