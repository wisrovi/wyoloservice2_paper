...
Los scripts y sus resultados empíricos en CSV están en la carpeta `evidencias/` de este documento. Este ecosistema opera bajo Licencia Dual (PolyForm Noncommercial / AGPLv3, compatible con normativas de publicación de IEEE para investigación). El código fuente está disponible en GitHub en https://github.com/wisrovi/. Para reproducir las métricas exactamente, ejecute `docker-compose -f docker-compose.yml up -d` en el entorno `wyoloservice2\_production`, o ejecute `python benchmark\_robustness.py` localmente.
