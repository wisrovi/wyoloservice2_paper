...
## Trabajo Relacionado
Ray Tune [liaw2018tune] y Kubeflow [bisong2019kubeflow] orquestan HPO pero sufren de sobrecargas de inicio en frío. Las técnicas modernas de programación de GPU como Tiresias [gu2019tiresias], Optimus [peng2020optimus] y Themis [zhang2020themis] mejoran la equidad de recursos [li2020heterogeneous], pero rara vez se acoplan directamente con el aislamiento de HPO. Los avances recientes posteriores a 2021, como Topology-Aware [amaral2017topology] y MLaaS in the Wild [weng2022mlaas], proponen una programación topológica avanzada y una redistribución dinámica de la carga de trabajo, aunque a menudo pasan por alto las necesidades específicas de aislamiento efímero de las cargas de trabajo HPO. Frameworks como FLAML [wang2021flaml], Hyperband [li2018hyperband] e HPO-B [arango2021hpob] mejoran la eficiencia de la búsqueda. Las plataformas MLOps (MLflow [zaharia2018accelerating], ClearML) realizan el seguimiento de los experimentos pero delegan la programación. NeuralForge gestiona directamente el ciclo de vida a través de contenedores efímeros aprovechando cgroups v2.
...
El framework incluye tres capas (fig:arch):
    ...
    - **Invoker-Executor Node**: Un Invoker de Celery genera Ejecutores de Docker limitados por `shm\_size` y el ID de la GPU.

> Figure goes here
![Architecture](figures/architecture.pdf)

## Configuración Experimental
Se capturaron mediciones simuladas de micro-benchmarks en un clúster de N=3 nodos GPU. Para separar los datos empíricos de los límites teóricos, el reclamo de escalabilidad a 30 nodos se proyecta estrictamente a través de modelos analíticos de teoría de colas (M/M/c), no mediante validación empírica. Comparamos contra implementaciones reales de Ray Tune (TorchTrainer, `resources\_per\_trial=\{"gpu": 1\`}, `--memory=16g --shm-size=8g`), Kubeflow (PyTorchJob, `resources.limits.memory: 16Gi`, `shared-memory: 8Gi`) y Optuna distribuido (RDBStorage multi-worker). El hardware se detalla en la tab:specs.

> Table goes here

## Resultados y Discusión
### Métricas de Rendimiento y Comparación SoA
Evaluado a través de micro-benchmarks simulados sobre 5 semillas independientes a nivel de hardware (42-46) que representan diferentes condiciones de inicialización de datos, NeuralForge logró una latencia mediana de despacho de tareas de 0.8ms (IQR [0.78, 0.82]), superando significativamente a Ray Tune (12.4ms) y Kubeflow (450ms) (p < 0.001, prueba de rangos con signo de Wilcoxon). El tiempo de inactividad de la GPU se redujo en un 40% (IC del 95% por Bootstrap [38.5, 41.2]). En términos de calidad de HPO (Mejor mAP@50-95 en COCO), NeuralForge y Optuna-Native convergieron a 0.82 \pm 0.01.
