...
\title{Technical Report: Shift-Left Data Gatekeeping for Computer Vision Datasets}
...
The transition from model-centric to data-centric AI emphasizes the critical role of data quality \cite{zha2023data}. Ng et al. \cite{ng2021data} demonstrated that early-stage quality assurance in annotation pipelines is vastly more cost-effective than late-stage algorithmic validation. Similarly, tools like Great Expectations \cite{greatexpectations2023} established benchmarks for data quality profiling at scale.

In distributed computing, Data Version Control \cite{kuprie2022dvc} identifies network-attached storage as a primary bottleneck when handling corrupt streams. Ephemeral containerization has proven effective for isolating validation tasks without burdening the host OS. 

Our architecture integrates these Data-Centric principles with modern Shift-Left paradigms, creating a deterministic validation gateway specific to YOLO architectures. This gatekeeper is a foundational component of the wisrovi-suit \cite{rodriguez2024wisrovi} distributed ecosystem.
...
\caption{A bar chart comparing the Wasted GPU Hours and Avg Debugging Time.}
