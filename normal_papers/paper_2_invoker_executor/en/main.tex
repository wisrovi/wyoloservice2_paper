...
\begin{enumerate}
    ...
    \item Execute \texttt{\footnotesize docker run --rm --gpus=all --memory=\$\{mem\_limit\} --cpus=\$\{nano\_cpus\} --shm-size=\$\{shm\_size\} wisrovi/train\_service:worker\_executor\_v1.0.0}.
...
\noindent Protocol: Boot latency defined as time from \texttt{\footnotesize docker run} to process ready state. Evaluated on uniform hardware.
\end{table}
...
This architecture operates under a Dual Licensing Model (PolyForm Noncommercial / AGPLv3). Generation scripts and code are available at \url{https://github.com/wisrovi/wyoloservice2_production}. Deployment is 100\% reproducible via \texttt{docker-compose up -d --build} to start the Invoker, which subsequently launches Executors via \texttt{docker run}. The provided CSV dataset (\texttt{data/production\_oom\_logs.csv}) is an aggregated empirical record derived directly from \texttt{cgroups} \texttt{memory.oom\_control}.
