...
def update_tex(lang):
    ...
    if lang == "en":
        ...
        old_res = r"\section{Results \& Discussion}"
        new_res = r"\section{Results \& Discussion}"
...
"""
        ...
        old_cluster = r"Clúster: tres nodos, cada uno con NVIDIA RTX 4090, 64 GB de RAM DDR5. Software: Celery 5.3, Docker 24.0, containerd 1.7, Kata Containers 3.0, gVisor, Firecracker 1.5, YOLOv8 \cite{ultralytics}. La multiplexación de GPU usa NVIDIA MPS \cite{nvidia_mps}. Los eventos OOM (Exit 137) se registraron cualitativamente vía \texttt{dmesg} y eventos de cgroups del kernel."
...
"""
    ...
    tex = tex.replace(old_cluster, new_cluster)
    
    # We find \subsection{Qualitative Observations: Ephemeral Isolation} and replace the whole section until \section{Data and Code Availability Statement}
    import re
    if lang == "en":
...
# Actually, I can just use my own script to generate markdown from latex to ensure byte-to-byte match.
import subprocess
subprocess.run(["python3", "generate_final_md_round8.py"], cwd=base_dir)

# Now fix markdown things manually that the generate script might miss
for lang in ["en", "es"]:
    ...
    if lang == "es":
        # First paragraph was missing in the python script regeneration perhaps?
